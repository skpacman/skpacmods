...
function skpacmod_utils.create_joker(def)
	joker_counter = joker_counter + 1
	def.id = joker_counter
	return def
end

function skpacmod_utils.getCardKey(card)
...
function skpacmod_utils.serialize(tbl, indent, visited)
	indent = indent or ""
	visited = visited or {}
	if visited[tbl] then
		return '"*RECURSION*"'
	...
	for k, v in pairs(tbl) do
		...
		if type(v) == "table" then
			value = skpacmod_utils.serialize(v, nextIndent, visited)
	...
	--append to ./logs/[datetime]-skpacmods.log
end

function skpacmod_utils.spawn_voucher(voucher_key)
